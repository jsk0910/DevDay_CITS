...
htmlTitle = """
  ...
  <div class="container title">
      ...
      <p style="font-weight: 600; font-size: 20px; color: #242DFC;">병원 최단 경로 도출</p>
  </div>
  <style type="text/css">
        @font-face {
...
if 'r' not in st.session_state or address != st.session_state.old_address:
  ...
  for _, row in df_hospital.iterrows():
    folium.Marker(location = [row['위도'], row['경도']],
            popup=folium.Popup(f"의료기관명: {row['의료기관명']}" + "<br/>" + f"전화번호: {row['전화번호']}", max_width=300),
            tooltip=row['의료기관명'],
            icon=folium.Icon(color='red',icon='plus')
          ).add_to(r)
  for i in range(3):
    folium.Marker(location = [min.iloc[i]['위도'], min.iloc[i]['경도']],
                  popup = folium.Popup(f"{min.iloc[i]['의료기관명']}" + "<br/>" + f"전화번호: {min.iloc[i]['전화번호']}",
                  max_width=300),
